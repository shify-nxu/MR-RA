...
ID_exposure['sei_score'] = sei_result_use_merge[ID_exposure['SNP_id']].abs()
...
sei_pick_variants_sort = ID_exposure_use.sort_values(by='sei_score',ascending=False)
sei_pick_variants_pick = sei_pick_variants_sort.loc[sei_pick_variants_sort['b_score'] > 0.5,:].iloc[0:7,:]


# Merge the differnces of same variant
sei_variants_result_extract = pd.concat([sei_result.iloc[:,5], sei_result.iloc[:,9:]], axis=1)
sei_pick_variants_result_extract_merge = sei_variants_result_extract.groupby('id').mean()

sei_pick_variants_result_extract  = sei_pick_variants_result_extract_merge.loc[sei_pick_variants_pick['SNP_id'],:]


# Add the b score of each variant to the merge result
sei_pick_variants_result_extract['b_score'] = ID_exposure_use.loc[sei_pick_variants_result_extract.index,'b_score']

sei_pick_variants_result_extract.to_csv("Merge-SNP-singleanalysis-Sei_pick_vairiants.txt",sep='\t')
