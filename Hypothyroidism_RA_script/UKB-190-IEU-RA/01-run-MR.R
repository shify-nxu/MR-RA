...
exp_dat <- extract_instruments(outcomes = 'ukb-a-190',clump = TRUE,r2 = 0.01,kb = 5000,access_token = NULL)

outcome_dat <- extract_outcome_data(exp_dat$SNP, outcomes = 'ebi-a-GCST000679', proxies = 1, rsq = 0.8, align_alleles = 1,  palindromes = 1, maf_threshold = 0.3)
...
dat$exposure <- "Treatment/medication code: levothyroxine sodium"
...
mr_result <- mr(dat)
mr_result_mre <- mr(dat,method_list=c('mr_ivw_mre'))
